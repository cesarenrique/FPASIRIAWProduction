...
<body>
  ...
  <ul>
    <li><a href="<?php echo url('/localidad/listar') ?>">Localidades</a></li>
    <li><a href="<?php echo url('/localidad/registrar') ?>">Registrar Localidad</a></li>
  </ul>
...
<table class="table">
    ...
    foreach ($todos as $fila) {
      // code...
          ...
          <td>{$fila['NOMBRE']}</td>
          <td>{$fila['CP']}</td>
          <td>{$fila['N_HABITANTE']}</td>
          <td>{$provincias[$i]}</td>
          <td>{$paises[$i]}</td>
					<td>
						<form id='formulario{$fila['id']}' action='". url('/localidad/modificarForm')."' method='post'>
