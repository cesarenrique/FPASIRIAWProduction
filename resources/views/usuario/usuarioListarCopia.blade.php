...
<body>
  ...
  <ul>
    <li><a href="<?php echo url('/usuario/listar') ?>">Usuario</a></li>
    <li><a href="<?php echo url('/usuario/registrar') ?>">Registrar Usuario</a></li>
  </ul>
...
<table class="table">
    ...
    foreach ($todos as $fila) {
      // code...
          ...
          <td>{$fila['APELLIDO']}</td>
          <td>{$fila['TELEFONO']}</td>
          <td>{$localidades[$i]}/{$provincias[$i]}/{$paises[$i]}</td>
					<td>
						<form id='formulario{$fila['id']}' action='". url('/usuario/modificarForm')."' method='post'>
